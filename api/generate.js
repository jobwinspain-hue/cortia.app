export default async function handler(req, res) {
  ...
  try {
    const { imageBase64, hairstyle, styleLabel } = req.body;
    if (!imageBase64 || !hairstyle) return res.status(400).json({ error: 'Faltan parámetros: imageBase64 y hairstyle son obligatorios' });
    ...
    // 2. Llamar a fal.ai hair-change (síncrono)
